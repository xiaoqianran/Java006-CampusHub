...
-- 兼容已创建过旧表的本地/CI Docker 数据卷。
ALTER TABLE t_resource
    ADD COLUMN IF NOT EXISTS summary VARCHAR(500) DEFAULT NULL COMMENT '资源摘要' AFTER description,
    ADD COLUMN IF NOT EXISTS content_markdown LONGTEXT DEFAULT NULL COMMENT 'Markdown 正文' AFTER summary,
    ADD COLUMN IF NOT EXISTS content_type VARCHAR(30) DEFAULT 'MARKDOWN' COMMENT '内容类型 MARKDOWN/HTML' AFTER content_markdown,
    MODIFY COLUMN file_url VARCHAR(500) DEFAULT NULL COMMENT '文件地址（可选，文字资源可为空）';
